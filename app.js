...
async function init() {
    try {
        ...
        } catch (e) { console.log("English not found"); }
        ...
        // 4. Attach buttons safely
        if (document.getElementById('search-btn')) document.getElementById('search-btn').onclick = toggleSearch;
        if (document.getElementById('audio-btn')) document.getElementById('audio-btn').onclick = playAudio;
        if (document.getElementById('share-btn')) document.getElementById('share-btn').onclick = shareChapter;
        if (document.getElementById('theme-btn')) document.getElementById('theme-btn').onclick = toggleTheme;
        if (document.getElementById('church-mode-btn')) document.getElementById('church-mode-btn').onclick = toggleChurch;

    } catch(e) {
        splash.innerHTML = "<h3>Error: " + e.message + "</h3><p>Check your GitHub 'data' folder and internet.</p>";
    }
}
    ...
    if (currentLibrary === 'saved') {
        saved.forEach(key => {
            ...
            if(verse) {
                const div = document.createElement('div');
                div.className = 'card';
                div.innerHTML = `<strong>${englishNames[b-1]} ${c}:${v}</strong><p>${verse.text}</p>`;
        ...
        for (const key in notes) {
            ...
            if(verse) {
                const div = document.createElement('div');
                div.className = 'card';
                div.innerHTML = `<strong>${englishNames[b-1]} ${c}:${v}</strong><p>${verse.text}</p><p class="note-text">${notes[key]}</p>`;
...
document.getElementById('search-input').addEventListener('input', (e) => {
    ...
    let html = '';
    results.forEach(v => {
        html += `<div class="card" onclick="jumpToVerse(${v.book},${v.chapter},${v.verse})"><strong>${englishNames[v.book-1]} ${v.chapter}:${v.verse}</strong><p>${v.text}</p></div>`;
    });
    document.getElementById('search-results').innerHTML = html;
});
function jumpToVerse(b,c,v) {
    currentBook = codes[b-1];
    currentBookName = englishNames[b-1];
    currentChapter = c;
    toggleSearch();
    loadChapter();
}
...
function playAudio() {
    ...
    const u = new SpeechSynthesisUtterance(currentEnglishChapterText);
    u.lang = 'en-US';
    speechSynthesis.speak(u);
}
